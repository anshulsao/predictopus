...
class Request {
    ...
    public static function forge($uri = null, $options = true, $method = null) {
        ...
        $request = new static($uri,
                isset($options['route']) ? $options['route'] : true, $method);
        if (static::$active) {
        ...
        \Event::instance()->has_events('request_created') and \Event::instance()->trigger('request_created',
                        '', 'none');
    ...
    public function __construct($uri, $route = true, $method = null) {
        ...
        logger(\Fuel::L_INFO,
                'Creating a new ' . (static::$main == null ? 'main' : 'HMVC') . ' Request with URI = "' . $this->uri->get() . '"',
                __METHOD__);
    ...
    public function execute($method_params = null) {
        ...
        \Event::instance()->has_events('request_started') and \Event::instance()->trigger('request_started',
                        '', 'none');
        ...
        try {
            if ($this->route->callable !== null) {
                $response = call_fuel_func_array($this->route->callable,
                        array($this));
                ...
                if (is_array($method_params)) {
                    $this->method_params = array_merge($this->method_params,
                            $method_params);
                }
                ...
                if ($class->hasMethod($method)) {
                    ...
                    \Event::instance()->has_events('controller_started') and \Event::instance()->trigger('controller_started',
                                    '', 'none');

                    $class->hasMethod('before') and $class->getMethod('before')->invoke($this->controller_instance);

                    $response = $action->invokeArgs($this->controller_instance,
                            $this->method_params);

                    $class->hasMethod('after') and $response = $class->getMethod('after')->invoke($this->controller_instance,
                            $response);
                    // fire any controller finished events
                    \Event::instance()->has_events('controller_finished') and \Event::instance()->trigger('controller_finished',
                                    '', 'none');
                } else {
                    throw new \HttpNotFoundException();
                }
        ...
        \Event::instance()->has_events('request_finished') and \Event::instance()->trigger('request_finished',
                        '', 'none');
